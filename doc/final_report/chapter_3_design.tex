...
this is achieved. These are the ideals that Packet Courier strives to embody, especially in light of objectives 4)
and 5).
...
\begin{itemize}
    ...
    has been generalised to a Poisson distribution. If a packet is dropped on the success of a Bernoulli
...
\lettrine{A}{lthough} Packet Courier would be able to provide value to users via APIs, it would likely alienate many
...
the simulation semantics further to integrate with any arbitrary packet, taking another step to evolve the tool once
more from a \emph{simulator} to an \emph{emulator}.
