...
\lettrine{W}{hilst} unit testing is often seen to be a menial aspect of ensuring robustness in software engineering
projects, unit tests in Packet Courier are used to verify the correctness of the statistical distribution
implementations, which is somewhat non-trivial. Each distribution is subjected to two different tests:
\begin{itemize}
...
\lettrine{P}{acket} Courier takes advantage of system testing to ensure that the aggregate properties of packets sent
over an emulated network accurately reflect those specified in the \texttt{.courierconfig} file. A selection of
...
data makes it through the timeline within a particular time frame.

\subsection{Analysis Methodology}\label{subsection:analysis_methodology}

\lettrine{E}{ach} client process runs the \texttt{analysis\_client.py} script which has been designed to send packets
that encapsulate all the information required to assess any transformations they may have undergone during transit.
...
\lettrine{T}{hree} sets of system testing data have been collected, each using different hardware. All three machines
were running a Ubuntu 20.04 LTS\cite{ubuntu_20_04}. Their specifications as per \texttt{sudo lshw -short} are listed in
...
\lettrine{S}{ix} participants were generous enough to spend some of their time to road test Packet Courier and provide
feedback. Four of the participants took part in the user testing phase remotely and expressed their feedback via
Microsoft Forms\cite{microsoft_forms}; the other two provided their feedback in person, which enables notes to be
taken as they were observed interacting with the tool in addition to writing down their comments.

\subsection{Survey Results}\label{subsection:survey_results}

\lettrine{P}{articipants} were asked to download a \texttt{.zip} file of the repository before compiling Packet Courier
and completing a selection of tasks. Each of the questions was carefully worded as not to invoke any biases and gave
only the bare minimum context for solving the particular problem. The rationale behind this is that Packet Courier's
...
\begin{enumerate}
    ...
    \item \textbf{Please read \texttt{cmd\_example1.courierconfig}. At first glance, does this file read intuitively
    ...
    weren't self-evident in their function?} \\
    ...
    develop for a set of fairly niche edge cases. It makes for a compelling suggestion, although some research into
...
\lettrine{T}{wo} user observation sessions were conducted to monitor how users \emph{behaved} while interacting with
Packet Courier before interviewing them to get an understanding for their thought processes\cite{user_observation}.
The first interview was done with ``John'' in a more informal manner in attempt to assess what users might expect
from the tool when reading the documentation and example configurations for the first time. The second interview was
done more formally with ``Harry'' to see how a user might debug a network protocol using Packet
Courier\footnote{``John'' and ``Harry'' are aliases being used to preserve anonymity.}.
...
\texttt{README} had to do with compiling the project. Comments like this highlight the fact that most developers take
this kind of information for granted and perhaps the \texttt{README} could be subtly tweaked to be friendlier to
those who are newer to the paradigm. Harry also mentioned that the initial build process spammed the console with
lots of \emph{``very intimidating''} text. This links back to some of the survey feedback from
...
though the \texttt{README} makes it too easy for all the various paths to be confused for one another and the
...
\lettrine{I}{n} light of all the quantitative evidence gathered during unit and system testing, and the qualitative
evidence presented using user testing iterations, Packet Courier can now be judged on how well it has completed the
objectives laid out in Section~\ref{section:objectives}.
...
Section~\ref{subsection:network_condition_semantics}, but has also gone further to incorporate event-based semantics,
