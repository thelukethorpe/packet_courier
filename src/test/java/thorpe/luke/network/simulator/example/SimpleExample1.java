...
import thorpe.luke.network.simulator.NodeState;
...
public class SimpleExample1 {
  ...
  public static void runNodeA(NodeState nodeState) {
    ...
    for (int i = 1; i <= 10; i++) {
      ...
      nodeState.getPostalService().mail(NODE_A_NAME, NODE_B_NAME, messageAsPacket);
    }
  }

  public static void runNodeB(NodeState nodeState) {
    ...
    Future<Void> result =
        executorService.submit(
            () -> {
              while (true) {
                Packet packet = nodeState.getMailbox().waitForMail();
